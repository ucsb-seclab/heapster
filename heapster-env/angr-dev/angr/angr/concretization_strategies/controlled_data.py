...
class SimConcretizationStrategyControlledData(SimConcretizationStrategy):
    ...
    def _concretize(self, memory, addr):
        ...
        symbolic_vars = filter(lambda key: not key.startswith("reg_") and not key.startswith("mem_") and not key.startswith("free_") and not key.startswith("malloc_") , memory.mem._name_mapping.keys())
        ...
        controlled_addrs = list(dict.fromkeys(controlled_addrs))
