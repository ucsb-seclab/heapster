...
class PropagatorState:
    def __init__(self, arch, replacements=None, mem_accesses=None, only_consts=False, prop_count=None, equivalence=None):
        self.arch = arch
        ...
        self._mem_accesses = list() if mem_accesses is None else mem_accesses
    ...
    def add_mem_access(self, codeloc, mem_access):
        self._mem_accesses.append(mem_access)
...
class PropagatorVEXState(PropagatorState):
    def __init__(self, arch, registers=None, local_variables=None, replacements=None, mem_accesses=None, only_consts=False,
                 prop_count=None):
        super().__init__(arch, replacements=replacements, mem_accesses=mem_accesses, only_consts=only_consts, prop_count=prop_count)
    ...
    def copy(self):
        cp = PropagatorVEXState(
            ...
            mem_accesses=self._mem_accesses.copy(),
...
class PropagatorAnalysis(ForwardAnalysis, Analysis):  # pylint:disable=abstract-method
    ...
    def _run_on_node(self, node, state):
        ...
        if self.mem_accesses is None:
            self.mem_accesses = state._mem_accesses
        else:
            self.mem_accesses.extend(state._mem_accesses)
