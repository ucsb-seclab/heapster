...
l = logging.getLogger("HHExecutor".ljust(23))
...
END_EXECUTION_FOUND_WINNING_FUNCTION = "END_EXECUTION_FOUND_WINNING_FUNCTION"


class HHExecutor(ExplorationTechnique):

    def __init__(self):
        super(HHExecutor, self).__init__()
    ...
    def successors(self, simgr, state:SimState, **kwargs):
        ...
        for ss in succs.flat_successors:
            ...
            if self.check_end_of_function(ss):
                l.debug("[!]    Reached end of function. Exiting.")
                break
            ...
            if ss.globals.get("out_of_rom", None):
                ...
                ss.callstack.ret() # here we need this to gracefully return from the continuation
