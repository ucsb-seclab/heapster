...
class HHSimgrViz(ExplorationTechnique):
    '''
    ...
    '''
    def __init__(self, cfg=None, hb_state=None):
        super(HHSimgrViz, self).__init__()
    ...
    def _add_state_to_graph(self, parent_state_id:str, sim_state_id:str, state:SimState):
        ...
        if self.cfg and state.addr in self.cfg.functions:
        ...
        #self._simgrG.nodes[sim_state_id]['peripherals_vars'] = str(state.state_dirty_things.peripherals_reads)
        #self._simgrG.nodes[sim_state_id]['svc_returns'] = str(state.state_dirty_things.svc_returns)
        #self._simgrG.nodes[sim_state_id]['constraints'] = str(state.solver.constraints)
        try:
            self._simgrG.nodes[sim_state_id]['step_time'] = str(state.globals["step_time"])
            self._simgrG.nodes[sim_state_id]['unsat_succs'] = str(state.globals["unsat_succs"])
    ...
    def successors(self, simgr, state:SimState, **kwargs):
        l.debug("[!]Seeing this {}".format(hex(state.addr)))
