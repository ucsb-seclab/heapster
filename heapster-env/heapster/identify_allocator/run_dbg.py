...
from .execute_heap_init import execute_hi, execute_forced_hi
...
def parse_opts():
    ...
    o.add_argument("--malloc-init", default=None)
    ...
    group_entry_point.add_argument('-hiks', '--heap_init_keep_symbolic', action='store_true', default=False)
    opts = o.parse_args()
    return opts
...
if __name__ == "__main__":
    ...
    if opts.malloc and opts.malloc_init:
        l.info("[+]Forced malloc to be {} and heap init to be {}".format(opts.malloc, opts.malloc_init))
        assert(opts.malloc in pointer_sources)
        pointer_sources = [opts.malloc]
        next_state = execute_forced_hi(project, 
                                       hb_state, 
                                       int(opts.malloc_init,16), 
                                       pointer_sources,
                                      )
        l.info("Last state of heap init at {}".format(hex(next_state.addr)))
        assert(next_state)
        res, ct_2 = is_ps_working(project, hb_state, int(opts.malloc,16), next_state)
        if res:
            l.info("[+] ✓ Specified pointer source at {} works as malloc with heap init {}".format(opts.malloc, opts.malloc_init))
            working_ps.append(WorkingPS(opts.malloc, ct_2, f, hi_init_state.memory.mem, True))
        else:
            l.info("[+] ✗ Specified pointer source at {} DOES NOT work as malloc with heap init {}".format(opts.malloc, opts.malloc_init))
    ...
    for psx_index, psx in enumerate(filtered_pointer_sources):
        ...
        for f_index, f in enumerate(potential_heap_initializers):
            ...
            if len(next_state.globals["symbolic_writes"]) == 0 or opts.heap_init_keep_symbolic:
