...
class IdentifiableReverseMemcpy(IdentifiableFunction):
    ...
    def __str__(self):
        return "IdentifiableReverseMemcpy"
...
def consistent_with_memcpy(project, func, result):
    ...
    length = args[2]
    ptr_dst, orig_dst = args[1].addr, args[1].data[:length]
    ptr_src, orig_src = args[0].addr, args[0].data[:length]
