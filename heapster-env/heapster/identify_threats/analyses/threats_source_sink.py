...
# Move this constant somewhere else to be parametric
MMIO_RANGE = (0x40000000, 0x60000000)

l = logging.getLogger("heapster.threat_checker_ssa")
l.setLevel(logging.CRITICAL)



class AnalysisTimerException(Exception):
    pass

class SourceSinkAnalysis():
    def __init__(self, project, bin_cfg, source_func, sink_func, sink_target_registers, 
                       scope=0, scope_function = None, valid_source= None, 
                       valid_sink=None, stop_at_mmio_defs = False, stop_at_undef_arg=False, timer= None):
        ...
        self.successfull_sinks = []

        self.stop_at_mmio_defs = stop_at_mmio_defs
        self.stop_at_undef_arg = stop_at_undef_arg
        
        self.timer = Timer(timer, self.timeout) if timer else timer
        self.timer_started = False


    def timeout(self):
        l.debug("[!] Timeout for analysis has been reached.")
        self.stop_analysis = True

    def run(self):
        l.debug("[S] Trying to find a match between {} retval and {} args".format(hex(self.source.addr), hex(self.sink.addr)))
        ...
        for sink_func_pred_addr, xrefs in sink_predecessors_dict.items():
            ...
            l.debug("[S] Now analyzing predecessor func at {}".format(hex(sink_func_pred_addr)))
            l.debug("[S] XRefs are {}".format((xrefs)))
            ...
            for xref in xrefs:
                ...
                l.debug("[S] Call to xref at {}".format(hex(call_to_xref_address)))
                ...
                if self.valid_sink == [] or call_to_xref_address in self.valid_sink:
                    ...
                    l.debug("[!S]  Discarding call {} because not valid sink".format(hex(call_to_xref_address)))
            
            if len(observation_points) == 0:
                l.info("[!]  No observation points. Returning.")
            else:
                for op in observation_points:
                    self.start_sink = op
                    l.info("[S]  Starting from xref at {}, target regs: {}".format(hex(op), self.sink_target_registers))
                    self._run(sink_func_pred, op, self.sink_target_registers)


    def _run(self, target_function, observation_point, target_registers):
        
        if self.timer != None and not self.timer_started:
        ...
        for sink_arg in target_registers:
            ...
            if len(reg_defs) == 0:
                l.debug("[!_S] No definition for register {}".format(sink_arg))
                l.debug("[!_S] Attempting to detect transition wrapper.")
                if self._is_wrapper(target_function):
                    l.debug("[!_S] Detected wrapper of function {}".format(hex(target_function.addr)))
                    l.debug("[!_S] Adding additional definitions to continue analysis at the callers.")
                    callers_to_analyze.add((hex(target_function.addr), Register(reg_offset,4)))
                else:
                    l.debug("[!_S]Could not detect wrapper, stopping research here at {}".format(hex(target_function.addr)))
            ...
            for definition in overall_defs:
                if definition[0] == "retval" and definition[1] != None:
                    l.debug("[_S] Definition is coming from retval of func {}".format(definition[1]))
                    if int(definition[1],16) == self.source.addr:
                        l.debug("[_S] Observed a flow from return value of {} to {}".format(hex(self.source.addr), hex(self.sink.addr)))
                        if self.valid_source != []:
                            if definition[2] in self.valid_source: 
                                ...
                                self.successfull_sinks.append(self.start_sink)
                                self.stop_analysis = True
                            else:
                                l.debug("[CL] Flow not at the codeloc requested. Continue.")
                        else:
                            self.observed_source_sink.add((definition[2], self.start_sink, self.current_sink_target_register))
                            self.successfull_sinks.append(self.start_sink)
                            self.stop_analysis = True
                    ...
                    l.debug("[_S] Definition is coming from arg {} of func {}".format(definition[1], hex(target_function.addr)))
                    if self.scope != 0:
                        callers_to_analyze.add((hex(target_function.addr), definition[1]))
                #elif definition[0] == "mmio-mem-const":
                #    if self.stop_at_mmio_defs:
                #        self.observed_source_sink.add(("mmio-def", self.start_sink, self.current_sink_target_register))
                #        self.stop_analysis = True
                else:
                    l.debug("[_S] Definition is coming from memory")
                    if definition[1] == None:
                        l.debug("[AC] Constant has no value!")
                        self.observed_arb_param.add(observation_point)
                        if self.stop_at_undef_arg:
                            self.stop_analysis = True
                    #self.observed_arb_param.add(observation_point)
    ...
    def analyze_call(self, funcs):
        ...
        while len(worklist) != 0 and not self.stop_analysis:
            ...
            if call_id in self.seen_call_ids:
                l.debug("[CL] Skipping call with already analyzed context")
                continue
            ...
            for endpoint_type, endpoint_blocknodes in func_td.endpoints_with_type.items():
                for eb in endpoint_blocknodes:
                    if endpoint_type == 'return':
                        ...
                        endpoint_cfg_node = self.project.cfg.model.get_any_node(eb.addr)
            ...
            for definition in overall_defs:
                if definition[0] == "retval":
                    ...
                    if definition[1] and int(definition[1],16) == self.source.addr:
                        l.debug("[CL] Observed a flow from return value of {} to {}".format(hex(self.source.addr), hex(self.sink.addr)))
                        if self.valid_source != []:
                            if definition[2] in self.valid_source: 
                                self.observed_source_sink.add((definition[2], self.start_sink, self.current_sink_target_register))
                                self.successfull_sinks.append(self.start_sink)
                    ...
                    l.debug("[CL] Definition coming from parameter of the function itself.")
                ...
                #        self.stop_analysis = True
                    ...
                    l.debug("[CL] Definition coming from constant.")
                    if definition[1] == None:
                        l.debug("[CL] Constant has no value!")
                        self.observed_arb_param.add(call_location)
                        if self.stop_at_undef_arg:
                            self.stop_analysis = True
    ...
    def analyze_caller(self, funcs):
        ...
        while len(worklist) != 0 and not self.stop_analysis:
            ...
            if type(current_function_arg) != Register:
                l.debug("Current function arg is not a Register")
                continue 
            ...
            for pred_function_addr, xrefs in func_predecessors_dict.items():
                ...
                for xref in xrefs:
                    l.debug("[AC] Analyzing XRefs at {} | Target register {}".format(hex(xref.addr), offset_to_reg(self.project, current_function_arg.reg_offset)))
                    ...
                    if len(rd.observed_results.values()) != 1:
                        l.debug("[!]  Observed results are {}. Continuing.".format(len(rd.observed_results.values())))
                        continue 
                    ...
                    if len(param_defs) == 0:
                        ...
                        if self._is_wrapper(start_function):
                            ...
                            l.debug("Could not detect wrapper, stopping research here at {}".format(hex(start_function.addr)))
                        ...
                        for definition in param_seen_defs:
                            if definition[0] == "retval":
                                ...
                                if definition[1] and definition[1] == self.source.addr:
                                    l.debug("[AC] Observed a flow from return value of {} to {}".format(hex(self.source.addr), hex(self.sink.addr)))
                                    if self.valid_source != []:
                                        if definition[2] in self.valid_source: 
                                            self.observed_source_sink.add((definition[2], self.start_sink, self.current_sink_target_register))
                                            self.successfull_sinks.append(self.start_sink)
                                            ...
                                            l.debug("Flow not at the codeloc requested. Continue.")
                                    else:
                                        self.observed_source_sink.add((definition[2], self.start_sink, self.current_sink_target_register))
                                        self.successfull_sinks.append(self.start_sink)
                                        self.stop_analysis = True
                            ...
                            #        self.stop_analysis = True
                                ...
                                # Found a constant/static definition
                                l.debug("[AC] Definition coming from constant.")
                                if definition[1] == None:
                                    l.debug("[AC] Constant has no value!")
        ...
        while len(defs_to_check) != 0:
            ...
            if def_value:
                ...
                if undefined_pointers:
                    ...
                    for v in dataset_data:
                        if type(v) == int and v >= MMIO_RANGE[0] and v <= MMIO_RANGE[1]:
                            def_value = ("mmio-mem-const", v)
                        else:
                            if type(v) == int:
                                def_value = ("const", v)
                            else:
