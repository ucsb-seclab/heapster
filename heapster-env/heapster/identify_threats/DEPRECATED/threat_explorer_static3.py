...
l = logging.getLogger("StaticHeapTransitionsHunter3")
l.setLevel("INFO")
...
'''
This is the threat identifier when using HeapHopper 
challenges as target.
'''
class StaticHeapTransitionsHunter3:
    
    def __init__(self, project, malloc, free, myread, mmio_nodes):
    ...
    def run(self):
        ...
        if calls_to_malloc_from_mmio is None:
            ...
            ssa = SourceSinkAnalysis(self.project, 
                                    self.project.cfg, 
                                    self.project.kb.functions[self.malloc],
                                    self.project.kb.functions[self.myread],
                                    ["r1"],
                                    scope=1, # just put a value !-0, doesn't matter what.
                                    valid_codeloc = valid_codelocs_source_sink
                                    )
