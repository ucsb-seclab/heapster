...
l = logging.getLogger("PointerSourceExecution".ljust(23))
...
class PointerSourceExecution(ExplorationTechnique):

    def __init__(self, current_hooks=None):
        super(PointerSourceExecution, self).__init__()
        ...
        # The list of functions we have already hooked becuase 
        # of timeout
        self.current_hooks = current_hooks

        # Executed functions and their cumulative time spent
        # emulating them.
        self.executed_funcs = {}
        self.start_func_time = 0

        # The current executed function
    ...
    def setup(self, simgr):
        ...
        self.current_function = func.addr 
        
        # Setting up skipping hooks
        for h in self.current_hooks:
            l.debug("Hooking function at {}".format(hex(h)))
            init_state.project.hook(addr=h, hook=skip())
    ...
    def successors(self, simgr, state:SimState, **kwargs):
        ...
        for ss in succs.flat_successors:
            ...
            assert(len(succs.flat_successors) == 1)
            ...
            # Register this function in the dictionary if it's the first time we see this.
            if current_function:
                self.current_function = current_function
                if self.current_function.addr not in self.executed_funcs.keys():
                    self.executed_funcs[self.current_function.addr] = 0

                prev_function = None
                if len(ss.history.bbl_addrs) > 1:
                    prev_function = ss.project.kb.functions.get(ss.history.addr, None)
                else:
                    l.critical("First executed function is {}".format(hex(current_function)))
                    # This is the first executed function if there is no history.
                    self.start_func_time = time.time()
                
                if prev_function and self.current_function.addr != prev_function.addr \
                                        and self.current_function.addr not in self.current_hooks:
                    end_time = time.time()
                    self.executed_funcs[current_function.addr] += (end_time - self.start_func_time)
                    self.start_func_time = time.time()
